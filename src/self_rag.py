...
def _llm():
    ...
    return ChatGroq(
        ...
        model=s.groq_model,      # default: openai/gpt-oss-120b
        temperature=0,
        max_retries=3,
    )

def _structured(schema):
    """Groq pe function-calling ki jagah native JSON schema mode use karta hai."""
    return _llm().with_structured_output(schema, method="json_schema")
...
def contextualize_question(state: RAGState):
    ...
    out = _structured(QueryRewrite).invoke(
...
def decide_retrieval(state: RAGState):
    ...
    out = _structured(RetrieveDecision).invoke(prompt.format_messages(question=state["question"]))
...
def grade_relevance(state: RAGState):
    ...
    grader = _structured(RelevanceDecision)
    relevant = []
...
def rewrite_internal_query(state: RAGState):
    ...
    out = _structured(QueryRewrite).invoke(prompt.format_messages(question=state["question"], previous=state.get("retrieval_query", "")))
...
def rewrite_web_query(state: RAGState):
    ...
    out = _structured(QueryRewrite).invoke(prompt.format_messages(question=state["question"], previous=state.get("web_query", "")))
...
def check_support(state: RAGState):
    ...
    out = _structured(SupportDecision).invoke(prompt.format_messages(question=state["question"], answer=state.get("answer", ""), context=state.get("context", "")))
...
def check_usefulness(state: RAGState):
    ...
    out = _structured(UsefulnessDecision).invoke(prompt.format_messages(question=state["question"], answer=state.get("answer", "")))
